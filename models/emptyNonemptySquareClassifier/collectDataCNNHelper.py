        ...
        for i in range(0, len(squares)):
            for j in range(0, len(squares[i])):
                ...
                if ((i == 0 or i == 7) and j <= 4) or ((i == 1 or i == 6) and j <= 1)  :
                    resizedImage.save(f"{nonemptySquareDestPath}/{imageCount}_nonemptySquare.png")
                    imageCount = imageCount + 1
                elif  (i == 2 or i == 3):
                    resizedImage.save(f"{emptySquareDestPath}/{imageCount}_emptySquare.png")
                    imageCount = imageCount + 1
...
for file in os.listdir(sourceDirectory):
    ...
    if ( fileName.endswith(".jpg") or fileName.endswith(".png") or fileName.endswith(".jpeg") ) :                 
        ...
        for image in emptySquareImages:
            image.save(f"{emptySquareDestPath}/{imageCount}_emptySquare.png")
            imageCount = imageCount + 1

        for image in nonemptySquareImages:
            image.save(f"{nonemptySquareDestPath}/{imageCount}_nonemptySquare.png")
            imageCount = imageCount + 1
